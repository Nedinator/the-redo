    ...
    if (interaction.customId.startsWith("banModal")) {
        ...
        if (!member.bannable) {
            return interaction.reply({
                content: "This user has moderator/admin permissions or higher permissions than this bot.",
                ephemeral: true,
            });
        }
        ...
        const memberDoc = await Member.findOne({ userID: targetId }).catch((err) => console.log(err));
        if (memberDoc) {
